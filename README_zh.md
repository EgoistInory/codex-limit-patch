...
日常使用不必保持浏览器打开。安装可选的 macOS 菜单栏组件：
...
菜单栏会每 60 秒在后台自动刷新，分别显示三个实时供应商的紧凑指标、各窗口重置
倒计时、告警状态、数据刷新时间和准确的数据来源。未配置凭据的可选供应商显示为
**Not configured**，不会触发全局告警。可以点击 **Refresh Now** 立即刷新，只有需要
完整详情时才点击 **Open Dashboard**。该进程不保存凭据，也不会监听本地网络端口。
